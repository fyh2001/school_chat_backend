...
var (
	...
	SucceedLogin  = result(10010, "登录成功")
	ErrUser       = result(10011, "用户不存在")
	ErrEmailCode  = result(10012, "邮箱验证码错误")
	ErrUserExist  = result(10013, "用户已存在")
	ErrUserAdd    = result(10014, "用户添加失败")
	ErrUserFind   = result(10015, "用户查找失败")
	ErrUserUpdate = result(10016, "用户更新失败")
	ErrUserDelete = result(10017, "用户删除失败")
